...
import layersBidafFusion as layBid_f
import layersQanet as layQ
import layersQanetOld as layQ_old
import torch
...
class BiDAF_fus(nn.Module):
    ...
    """
    def __init__(self, word_vectors, char_vectors,  char_emb_dim, hidden_size, drop_prob=0.):
        super(BiDAF_fus, self).__init__()
        self.emb = layBid_f.Embedding(word_vectors=word_vectors,
                                    char_vectors=char_vectors,
                                    char_emb_dim=char_emb_dim,
                                    hidden_size=hidden_size,
                                    drop_prob=drop_prob)

        self.enc = layBid_f.RNNEncoder(input_size=hidden_size,
                                     hidden_size=hidden_size,
                                     num_layers=1,
                                     drop_prob=drop_prob)

        self.att = layBid_f.BiDAFAttention(hidden_size=2 * hidden_size,
                                         drop_prob=drop_prob)

        self.mod = layBid_f.RNNEncoder(input_size=8 * hidden_size,
                                     hidden_size=hidden_size,
                                     num_layers=2,
                                     drop_prob=drop_prob)

        self.out = layBid_f.BiDAFOutput(hidden_size=hidden_size,
                                      drop_prob=drop_prob)
...
class QANet(nn.Module):
    ...
    def __init__(self, word_vectors, char_vectors, char_emb_dim,hidden_size=128,
        ...
        self.emb_enc = nn.ModuleList([layQ.EncoderBlock(n_conv=n_conv_emb_enc,
        ...
        self.mod_enc = nn.ModuleList([layQ.EncoderBlock(n_conv=n_conv_mod_enc,
                                     hidden_size=hidden_size,
                                     drop_prob=drop_prob_word,
...
class QANet_independant_encoder(nn.Module):
    """Baseline QANet model for SQuAD.
    ...
    def __init__(self, word_vectors, char_vectors, char_emb_dim,hidden_size=128,
                 ...
                 kernel_size_emb_enc_block=7, kernel_size_mod_enc_block=7,
                 n_heads=8, divisor_dim_kqv=8):
        super(QANet_independant_encoder,self).__init__()
        self.n_emb_enc_blocks=n_emb_enc_blocks
        self.n_mod_enc_blocks=n_mod_enc_blocks
        self.n_conv_emb_enc=n_conv_emb_enc
        self.n_conv_mod_enc=n_conv_mod_enc
        ...
        self.mod_enc_1 = nn.ModuleList([layQ.EncoderBlock(n_conv=n_conv_mod_enc,
                                     hidden_size=hidden_size,
                                     drop_prob=drop_prob_word,
                                     kernel_size=kernel_size_mod_enc_block, n_heads=n_heads, 
                                     divisor_dim_kqv=divisor_dim_kqv) for _ in range(n_mod_enc_blocks)])
        
        self.mod_enc_2 = nn.ModuleList([layQ.EncoderBlock(n_conv=n_conv_mod_enc,
                                     hidden_size=hidden_size,
                                     drop_prob=drop_prob_word,
                                     kernel_size=kernel_size_mod_enc_block, n_heads=n_heads, 
                                     divisor_dim_kqv=divisor_dim_kqv) for _ in range(n_mod_enc_blocks)])
        
        self.mod_enc_3 = nn.ModuleList([layQ.EncoderBlock(n_conv=n_conv_mod_enc,
                                     hidden_size=hidden_size,
                                     drop_prob=drop_prob_word,
                                     kernel_size=kernel_size_mod_enc_block, n_heads=n_heads, 
                                     divisor_dim_kqv=divisor_dim_kqv) for _ in range(n_mod_enc_blocks)])
    ...
    def forward(self, cw_idxs,cc_idxs, qw_idxs, qc_idxs):
        ...
        for i,block in enumerate(self.mod_enc_1):
            att=block(att,c_mask,i*(self.n_conv_mod_enc+2)+1,self.total_layers_mod_enc)
        M0=att # (batch_size, c_len, hidden_size)
        for i,block in enumerate(self.mod_enc_2):
            att=block(att,c_mask,i*(self.n_conv_mod_enc+2)+1,self.total_layers_mod_enc)
        M1=att # (batch_size, c_len, hidden_size)
        for i,block in enumerate(self.mod_enc_3):
            att=block(att,c_mask,i*(self.n_conv_mod_enc+2)+1,self.total_layers_mod_enc)
        M2=att # (batch_size, c_len, hidden_size)
...
class QANet_old(nn.Module):
    ...
    Args:
        ...
        n_heads (int): Number of heads in the self-attention mechanisms
    """
    def __init__(self, word_vectors, char_vectors, device,hidden_size=128,char_emb_dim=200,
                 n_emb_enc_blocks=1,n_mod_enc_blocks=7,
                 n_conv_emb_enc=4,n_conv_mod_enc=2,
                 drop_prob_word=0.1,drop_prob_char=0.05,
                 kernel_size_emb_enc_block=7, kernel_size_mod_enc_block=7,
                 n_heads=8):
        super(QANet_old,self).__init__()
        self.n_emb_enc_blocks=n_emb_enc_blocks
        self.n_mod_enc_blocks=n_mod_enc_blocks
        self.n_conv_emb_enc=n_conv_emb_enc
        self.n_conv_mod_enc=n_conv_mod_enc
        ...
        self.emb = layQ_old.EmbeddingQANet(word_vectors=word_vectors,
                                         char_vectors=char_vectors,
                                         char_emb_dim=char_emb_dim,
                                         hidden_size=hidden_size,
                                         drop_prob_word=drop_prob_word,
        ...
        self.emb_enc = nn.ModuleList([layQ_old.EncoderBlock(n_conv=n_conv_emb_enc,
                                     device=device,
                                     hidden_size=hidden_size,
                                     drop_prob=drop_prob_word,
        ...
        self.att = layQ_old.BiDAFAttention(hidden_size=hidden_size,
                                         drop_prob=drop_prob_word)
        
        self.resize_layer = nn.Conv1d(4 * hidden_size, hidden_size, kernel_size=1)

        self.mod_enc = nn.ModuleList([layQ_old.EncoderBlock(n_conv=n_conv_mod_enc,
                                     device=device,
                                     hidden_size=hidden_size,
                                     drop_prob=drop_prob_word,
        ...
        self.out = layQ_old.QANetOutput(hidden_size=hidden_size)
